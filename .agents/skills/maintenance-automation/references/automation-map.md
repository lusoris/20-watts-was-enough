...
| Labels, roadmap milestones, issue assignments and linked pull-request metadata | `.github/labels.json`, `.github/milestones.json`, `.github/issue-milestones.json`, the managed issue's classifications and one explicit pull-request body reference | `go -C tooling run ./cmd/20w github sync-metadata --root .. --check` and `go -C tooling run ./cmd/20w github sync-pr-metadata --root .. --check` | Trusted-main workflows or explicit repository commands; the issue map projects one unambiguous reference, while missing or competing managed references cause no issue-derived pull-request write |
