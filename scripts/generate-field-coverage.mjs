...
const euroscivocRoutes = routing.assignments.filter(
  (assignment) => assignment.level === "euroscivoc-concept",
);
const euroscivocCounts = {
  dedicated: euroscivocRoutes.filter((assignment) => assignment.state === "dedicated").length,
  adjacent: euroscivocRoutes.filter((assignment) => assignment.state === "adjacent").length,
  unreviewed: data.taxonomies.euroscivoc.declaredConcepts - euroscivocRoutes.length,
};
...
markdown.push(
  ...
  "",
  "![Multi-resolution EU, OECD, DFG, and ANZSRC research-field coverage](../public/plots/global-field-coverage.svg)",
  "",
  ...
  "",
  `The EU-level EuroSciVoc inventory contributes **${data.taxonomies.euroscivoc.declaredConcepts.toLocaleString("en-US")} multilingual concepts** under ${data.taxonomies.euroscivoc.declaredTopConcepts} top branches. It is a discovery and routing vocabulary, not a coverage score: every concept remains unassigned until the [fine-grained routing ledger](taxonomies/field-depth.md) names a field-centered audit and evidence boundary.`,
  "",
  `The finer DFG probe contains **${data.dfgReviewBoards.length} review boards and ${data.taxonomies.dfg.declaredSubjects} subjects**. At review-board resolution, the repository has ${dfgCounts.dedicated} dedicated, ${dfgCounts.adjacent} adjacent, and ${dfgCounts.unreviewed} unreviewed areas. That higher apparent coverage is not greater depth: one audit can touch a large review board while leaving most of its constituent subjects untouched. Every named subject is now preserved in the [fine-grained field-depth inventory](taxonomies/field-depth.md), where parent labels are explicitly prevented from propagating.`,
  "",
  `The independent ANZSRC census contains **${data.anzsrcDivisions.length} divisions, ${data.taxonomies.anzsrc.declaredGroups} groups, and ${data.taxonomies.anzsrc.declaredFields.toLocaleString("en-US")} fields**. At the deliberately coarse division level, ${anzsrcCounts.dedicated} have a dedicated entry audit, ${anzsrcCounts.adjacent} have adjacent evidence only, and ${anzsrcCounts.unreviewed} are wholly unreviewed. This is a disagreement detector, not evidence that the ${data.taxonomies.anzsrc.declaredGroups} groups or ${data.taxonomies.anzsrc.declaredFields.toLocaleString("en-US")} fields have been audited. The complete corrected hierarchy is searchable in the [fine-grained field-depth inventory](taxonomies/field-depth.md).`,
  "",
  ...
  "## Taxonomy contract",
  "",
  `- The EU discovery inventory is [${data.taxonomies.euroscivoc.title}](${data.taxonomies.euroscivoc.url}), ${data.taxonomies.euroscivoc.edition}: ${data.taxonomies.euroscivoc.declaredConcepts.toLocaleString("en-US")} multilingual concepts. It supplies search terms and hierarchy, not inherited coverage.`,
...
markdown.push(
  ...
  "",
  "The DFG layer catches gaps hidden by the broader OECD cells. It is shown here at review-board level; all 214 individual subjects are enumerated separately in the [fine-grained field-depth inventory](taxonomies/field-depth.md), but still require child-level routing assignments.",
  "",
);
...
markdown.push(
  ...
  "3. **Computing and engineering remainder:** audit operating systems, distributed systems, runtime/language breadth, empirical software engineering, accessible visualization, hardware ageing, device security, and lifecycle qualification with executable boundaries.",
  ...
  return groups
    .map((group, index) => {
      ...
      const segments = ["dedicated", "adjacent", "unreviewed"]
        ...
        .join("");
      return `<text x="${x}" y="${rowY + 25}" fill="${colors.text}" font-family="Segoe UI, sans-serif" font-size="13">${esc(group.code)} · ${esc(group.name)}</text>${segments}<text x="${barX + barWidth + 10}" y="${rowY + 25}" fill="${colors.muted}" font-family="Cascadia Mono, monospace" font-size="12">${group.total}</text>`;
    })
    .join("");
}
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="1160" viewBox="0 0 1200 1160" role="img" aria-labelledby="title description">
  ...
  <rect x="36" y="132" width="1128" height="96" rx="16" fill="${colors.panel}" stroke="${colors.grid}"/>
  <text x="62" y="170" fill="${colors.text}" font-family="Georgia, serif" font-size="22" font-weight="700">EU EuroSciVoc 1.6 · 1,064 multilingual concepts</text>
  <text x="62" y="201" fill="${colors.muted}" font-family="Cascadia Mono, monospace" font-size="13">${euroscivocCounts.dedicated} dedicated routes · ${euroscivocCounts.adjacent} adjacent routes · ${euroscivocCounts.unreviewed} unassigned concepts</text>
  ${renderOverallBar(euroscivocCounts, data.taxonomies.euroscivoc.declaredConcepts, { x: 730, y: 168, width: 385 })}
  <rect x="36" y="248" width="1128" height="378" rx="16" fill="${colors.panel}" stroke="${colors.grid}"/>
  <text x="62" y="294" fill="${colors.text}" font-family="Georgia, serif" font-size="23" font-weight="700">OECD FORD · 42 global second-level fields</text>
  <text x="62" y="323" fill="${colors.muted}" font-family="Cascadia Mono, monospace" font-size="13">${oecdCounts.dedicated} dedicated · ${oecdCounts.adjacent} adjacent · ${oecdCounts.unreviewed} unreviewed</text>
  ${renderRows(oecdGroups, { x: 62, y: 346, width: 1015, rowHeight: 43, maxTotal: 11 })}
  <rect x="36" y="648" width="1128" height="244" rx="16" fill="${colors.panel}" stroke="${colors.grid}"/>
  <text x="62" y="694" fill="${colors.text}" font-family="Georgia, serif" font-size="23" font-weight="700">DFG 2024–2028 · 49 review boards / 214 subjects</text>
  <text x="62" y="723" fill="${colors.muted}" font-family="Cascadia Mono, monospace" font-size="13">${dfgCounts.dedicated} dedicated · ${dfgCounts.adjacent} adjacent · ${dfgCounts.unreviewed} unreviewed</text>
  ${renderRows(dfgGroups, { x: 62, y: 746, width: 1015, rowHeight: 36, maxTotal: 19 })}
  <rect x="36" y="910" width="1128" height="82" rx="16" fill="${colors.panel}" stroke="${colors.grid}"/>
  <text x="62" y="944" fill="${colors.text}" font-family="Georgia, serif" font-size="21" font-weight="700">ANZSRC 2020 · 23 divisions / 213 groups / 1,967 fields</text>
  <text x="62" y="972" fill="${colors.muted}" font-family="Cascadia Mono, monospace" font-size="13">${anzsrcCounts.dedicated} dedicated · ${anzsrcCounts.adjacent} adjacent · ${anzsrcCounts.unreviewed} unreviewed at division resolution</text>
  ${renderOverallBar(anzsrcCounts, data.anzsrcDivisions.length, { x: 730, y: 942, width: 385 })}
  <rect x="36" y="1010" width="1128" height="82" rx="16" fill="${statePanels[leadDivergence.state]}" stroke="${colors[leadDivergence.state]}"/>
  <text x="62" y="1044" fill="${colors[leadDivergence.state]}" font-family="Cascadia Mono, monospace" font-size="13" font-weight="700">CROSS-TAXONOMY SIGNAL · ${esc(leadDivergence.taxonomy)} ${esc(leadDivergence.code)}</text>
  <text x="62" y="1072" fill="${colors.text}" font-family="Georgia, serif" font-size="20" font-weight="700">${esc(leadDivergence.name)} — ${esc(stateLabels[leadDivergence.state])}; not separately visible in OECD / DFG</text>
  <text x="56" y="1136" fill="${colors.muted}" font-family="Segoe UI, sans-serif" font-size="12">Parent-level green does not propagate. EuroSciVoc concepts, DFG subjects, ANZSRC groups, and ANZSRC fields require explicit routing.</text>
</svg>\n`;
...
if (checkOnly) {
  ...
  console.log(`Field coverage validated: ${data.taxonomies.euroscivoc.declaredConcepts} EuroSciVoc concepts, ${data.oecdFields.length} OECD fields, ${data.dfgReviewBoards.length} DFG review boards, and ${data.anzsrcDivisions.length} ANZSRC divisions.`);
} else {
  ...
  console.log(`Generated ${path.relative(root, markdownPath)} and ${path.relative(root, plotPath)} from four taxonomy lenses.`);
}
